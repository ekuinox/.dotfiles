...
- ソースディレクトリ `~/.dotfiles` を git リポジトリとし、GitHub の private リポジトリでホストする（個人メール等を含むため public にはしない）。
...
1. chezmoi を mise で導入する（`mise use -g chezmoi`）。グローバル mise 設定に宣言され、設定ごと同期される。
